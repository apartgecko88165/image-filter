...
class Filter(tk.Tk):
    ...
    def _save(self, image):
        ...
        image.save(f"{directory}/{fname}.png")
    ...
    def _show(self, ph, clicked):
        ...
        if self.clicked.get() == "Grayscale":
            ...
            self.disp.save("temp/photo.png")

            display = ImageTk.PhotoImage(file="temp/photo.png")

            self.canvas.configure(image=display)
            self.canvas.image = display
        elif self.clicked.get() == "Greenscale":
            self._resize(self.image)

            self.disp = self._green(self.disp)
    ...
    def start(self, image):
        ...
        tk.Button(self, text="SAVE", command=lambda: self._save(self.image)).place(x=self.disp.width-40, y=self.disp.height-30)
